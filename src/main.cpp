...
//    n.x = 100;   // for RTOS
  ...
  // BlueTooth:
    ...
    digitalWrite(WHEEL_L_DIRECTION, L); oDirL = directionL = +1;
    digitalWrite(WHEEL_R_DIRECTION, H); oDirR = directionR = +1;
...
/*    if (text.startsWith("+")) */
   ...
   if (flag == 1)
   {
        ...
        if (vL > 255) vL = 255;
        if (vL < 0)  vL = 0; // sollte 0 sein - zum debuggen besser... 
        ...
        if (vR > 255) vR = 255;
        if (vR < 0)  vR = 0; // sollte 0 sein - zum debuggen besser... 
...
void driveControl(void) // speed, diff
{
    ...
    {
      amountL = -setL;
      digitalWrite(WHEEL_L_DIRECTION, H); directionL = -1;
    }
    ...
    {
      amountR = -setR;
      digitalWrite(WHEEL_R_DIRECTION, L); directionL = -1;
    }
...
void IRAM_ATTR onTimer(void)
{
    ...
    if (((tick << 2) & 0xff) > vL) digitalWrite(WHEEL_L, L); else digitalWrite(WHEEL_L, H);
    if (((tick << 2) & 0xff) > vR) digitalWrite(WHEEL_R, L); else digitalWrite(WHEEL_R, H);
